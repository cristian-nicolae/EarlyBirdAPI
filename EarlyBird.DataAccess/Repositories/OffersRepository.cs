...
namespace EarlyBird.DataAccess.Repositories
{
    public class OffersRepository : IOffersRepository
    {
        ...
        {
            return context.Offers.Include(x => x.Location).FirstOrDefault(x => x.Id == id);
        }
        ...
        public IEnumerable<OfferEntity> GetAllOffers(OfferFilterAndSortDAO query)
        {
            ...
            if (query.CategoryId.HasValue)
                ...
                .Any(x => x.CategoryId == query.CategoryId));

            if (!String.IsNullOrWhiteSpace(query.City))
                offerQuery = offerQuery.Where(x => x.Location.CityName.Contains(query.City));
